...
export class OverlayState implements OnDestroy {
    ...
    async addContacts(contact: ContactList) {
        contact.firstName =
            ...
            contact.firstName.slice(1); // contact.firstName.slice(1) to add the rest of the name!
        contact.lastName =
    ...
    mobileViewContacts(activeUser: number) {
        ...
        const contactListRef = document.querySelector(
            '.contact-list-component'
        );
        const infoRef = document.querySelector(
            '.app-info-screen-mobile-component'
        );
    ...
    controllResize() {
        window.addEventListener('resize', () => {
            // 'resize' JS implemented eventlistener!
            const contactListRef = document.querySelector(
                '.contact-list-component'
            );
            const infoRef = document.querySelector(
    ...
    editSplitFullName(fullName: string, target: ContactList) {
        ...
        target.lastName =
            lastParts.join(' ').charAt(0).toUpperCase() +
            lastParts.join(' ').slice(1);
        target.initials =
